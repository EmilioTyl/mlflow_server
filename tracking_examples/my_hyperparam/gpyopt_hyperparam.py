...
"""
Main idea:
Create an eval function that registrates each evalauation of the bayesian opt as a nested run, it uses MlFlow 
to run the train entry poinnt,and it returns the evalaution loss by inspecting the historic registred metrics 
...
def run(training_data, max_runs, batch_size, max_p, epochs, metric, gpy_model, gpy_acquisition, initial_design, seed):
    bounds = [
    ...
    # Use tracking_client for accesing the loss values of the run
    

    #null_xx_loss is the  maximum reported loss, because GPyOpt does not handle inf
    def new_eval(nepochs,
        ...
        def eval(params):
            ...
            with mlflow.start_run(nested=True) as child_run:
                ...
                train(training_data = training_data, epochs =  nepochs,
                        batch_size = 16, learning_rate = lr , momentum= momentum,
                        seed=seed)
                ...
                succeeded = True
            if succeeded:
                training_run = tracking_client.get_run(run_id)
